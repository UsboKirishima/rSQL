...
static inline int lexIsKeyword(struct lexer_t *lexer) {
    return lexer->current_token.type >= 0x2000 && lexer->current_token.type < 0x3000;
}

/* Check if current token is an operator */
static inline int lexIsOperator(struct lexer_t *lexer) {
    return lexer->current_token.type >= 0xf000 && lexer->current_token.type < 0x1000;
}

/* Check if current token is punctuation */
static inline int lexIsPunctuation(struct lexer_t *lexer) {
    return lexer->current_token.type >= 0x1000 && lexer->current_token.type < 0x2000;
}
